...
def page_chefs():
    ...
    with st.expander("➕ Créer un nouveau chef de projet"):
        with st.form("form_nouveau_chef"):
            ...
            commentaires = st.text_area("Commentaires")
            st.caption("ℹ️ La capacité max (ICC) est calculée automatiquement à partir des critères ci-dessus, elle n'est pas saisie manuellement.")
    ...
    for idx, chef in chefs_display.iterrows():
        ...
        if st.session_state.get(f"editing_chef_{chef_id}", False):
            with st.form(f"form_edit_chef_{chef_id}"):
                ...
                exp_e = st.number_input("Années d'expérience", min_value=0, max_value=50, value=int(chef.get('Annees_Experience', 0) or 0))

                tech_opts = ["2=Basique", "3=Moyen", "4=Bon", "5=Excellent"]
                mgmt_opts = ["2=Débutant", "3=Moyen", "4=Bon", "5=Excellent"]
                ia_opts = ["1=Aucune", "2=Occasionnelle", "3=Régulière", "4=Quotidienne", "5=Avancée"]

                def _idx(opts, val, default=0):
                    return opts.index(val) if val in opts else default

                tech_e = st.selectbox("Compétences techniques", tech_opts, index=_idx(tech_opts, chef.get('Competences_Tech')))
                mgmt_e = st.selectbox("Compétences management", mgmt_opts, index=_idx(mgmt_opts, chef.get('Competences_Mgmt')))
                ia_e = st.selectbox("Utilisation IA", ia_opts, index=_idx(ia_opts, chef.get('Utilisation_IA')))
                secteurs_e = st.text_input("Secteurs d'expertise", value=chef.get('Secteurs_Expertise', '') or '')
                ...
                commentaires_e = st.text_area("Commentaires", value=chef.get('Commentaires', '') or '')

                st.caption("ℹ️ La capacité max (ICC) sera recalculée automatiquement à partir des critères ci-dessus.")
                ...
                if save:
                    ...
                    algo = AlgorithmeAffectationV5(ponderations)
                    criteres_icc = {
                        'Competences_Mgmt': mgmt_e, 'Annees_Experience': exp_e,
                        'Competences_Tech': tech_e, 'Utilisation_IA': ia_e
                    }
                    icc_recalcule = algo.calculer_icc(criteres_icc)

                    data = {
                        'Nom_Prenom': nom_e, 'Email': email_e, 'Annees_Experience': exp_e,
                        'Competences_Tech': tech_e, 'Competences_Mgmt': mgmt_e, 'Utilisation_IA': ia_e,
                        'Secteurs_Expertise': secteurs_e, 'Commentaires': commentaires_e,
                        'Capacite_Max': icc_recalcule, 'ICC_H_Semaine': round(icc_to_heures_semaine(icc_recalcule), 1),
                        'Statut': statut_e
                    }
                    if dm.modifier_chef(chef_id, data):
                        cached_get_chefs.clear()
                        st.success(f"✅ Modifié (ICC recalculé = {icc_recalcule})")
